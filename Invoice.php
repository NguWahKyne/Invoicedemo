...
class Invoice{
	...
	public function saveInvoice($POST) {		
		$sqlInsert = "
			...
			VALUES ('".$POST['userId']."', '".$POST['Name']."', '".$POST['address']."', '".$POST['subTotal']."', '".$POST['taxAmount']."', '".$POST['taxRate']."', '".$POST['totalAftertax']."', '".$POST['amountPaid']."', '".$POST['amountDue']."', '".$POST['notes']."')";		
		...
		for ($i = 0; $i < count($POST['item_code']); $i++) {
			...
			VALUES ('".$lastInsertId."','".$POST['item_code'][$i]."', '".$POST['item_name'][$i]."', '".$POST['quantity'][$i]."', '".$POST['price'][$i]."', '".$POST['total'][$i]."')";			
	...
	public function updateInvoice($POST) {
		if($POST['invoiceId']) {	
			$sqlInsert = "
				...
				SET order_receiver_name = '".$POST['Name']."', order_receiver_address= '".$POST['address']."', order_total_before_tax = '".$POST['subTotal']."', order_total_tax = '".$POST['taxAmount']."', order_tax_per = '".$POST['taxRate']."', order_total_after_tax = '".$POST['totalAftertax']."', order_amount_paid = '".$POST['amountPaid']."', order_total_amount_due = '".$POST['amountDue']."', note = '".$POST['notes']."' 
		...
		for ($i = 0; $i < count($POST['item_code']); $i++) {			
			$sqlInsertItem = "
				INSERT INTO ".$this->invoiceOrderItemTable."(order_id,item_code, item_name, order_item_quantity, order_item_price, order_item_final_amount) 
				VALUES ('".$POST['invoiceId']."','".$POST['item_code'][$i]."', '".$POST['item_name'][$i]."', '".$POST['quantity'][$i]."', '".$POST['price'][$i]."', '".$POST['total'][$i]."')";			
			mysqli_query($this->dbConnect, $sqlInsertItem);			
		}       	
	}	
